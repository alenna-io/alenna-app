...
export default function SchoolInfoPage() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const isSchoolAdmin = userData.roles.some((role: { name: string }) => role.name === 'SCHOOL_ADMIN');
        const canViewStudents = userData.permissions.includes('students.read');
        // School admins should see teacher counts even if they lack explicit users.read permission
        const canViewTeachers = userData.permissions.includes('users.read') || isSchoolAdmin;
        ...
        if (isSuperAdmin || canViewTeachers) {
          ...
          try {
            // For superadmins, allow specifying any school ID. For school admins, use /me to ensure consistent behavior
            const teachersCountData = isSuperAdmin
  ...
  // School admins should always see the teachers management card, even if they lack explicit users.read permission
  const canViewTeachers = !isSuperAdmin && ((userInfo?.permissions.includes('users.read') ?? false) || isSchoolAdmin);
      ...
      {school && (
        <>
          <div className="space-y-6">
            ...
            <div className='flex flex-wrap lg:grid grid-cols-2 gap-2'>
              {(canViewStudents || isSuperAdmin) && (
                <Card className='w-screen lg:w-auto'>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                          ...
                          {loadingStudentsCount ? (
                            ...
                            <p className="text-2xl font-bold text-blue-600">
                              ...
                              {school.userLimit !== undefined && (
                                <span className="text-base text-muted-foreground">/{school.userLimit}</span>
                              )}
                            </p>
                          )}
                        </div>
                          ...
                          {loadingTeachersCount ? (
                            ...
                            <p className="text-2xl font-bold text-green-600">
                              ...
                              {school.teacherLimit !== undefined && (
                                <span className="text-base text-muted-foreground">/{school.teacherLimit}</span>
                              )}
                            </p>
                          )}
                        </div>
